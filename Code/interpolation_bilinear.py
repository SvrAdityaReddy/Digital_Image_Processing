...
interpolated = cv2.resize(small,(0,0) ,fx=2,fy=2,interpolation=cv2.INTER_LINEAR)
small2 = cv2.resize(img2_grey, (0,0), fx=0.5, fy=0.5)
interpolated2 = cv2.resize(small2,(0,0) ,fx=2,fy=2,interpolation=cv2.INTER_LINEAR) 
# print small.shape
print interpolated.shape

cv2.imwrite("/home/aditya/diptemp/cameraman_bilinear.png",interpolated)
cv2.imwrite("/home/aditya/diptemp/lena_bilinear.jpg",interpolated2)


energy_original = np.sum(img,dtype=np.int32)
energy_interpolated = np.sum(interpolated,dtype=np.int32)
Energy_Difference = energy_original - energy_interpolated
print "Interpolation using bilinear"
